...
def load_entire_file(filename):
    x = _f.mitie_load_entire_file(ctypes.create_string_buffer(filename.encode('utf-8')))
    if (x == None):
...
class named_entity_extractor:
    def __init__(self, filename):
        ...
        if (isinstance(filename, ctypes.c_void_p)):
            ...
            self.__obj = _f.mitie_load_named_entity_extractor(ctypes.create_string_buffer(filename.encode('utf-8)')))
        if (self.__obj == None):
...
class binary_relation_detector:
    def __init__(self, filename):
        ...
        if (isinstance(filename, ctypes.c_void_p)):
            ...
            self.__obj = _f.mitie_load_binary_relation_detector(ctypes.create_string_buffer(filename.encode('utf-8')))
        if (self.__obj == None):
